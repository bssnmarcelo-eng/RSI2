...
export function BacktestWorkbench() {
  ...
  const [restrictToIndex, setRestrictToIndex] = useState(false);
  ...
  return <div>
    <Tabs value={mode} onValueChange={(value) => setMode(value as BacktestMode)} className="mb-6"><TabsList><TabsTrigger value="portfolio">Carteira</TabsTrigger><TabsTrigger value="per_asset">Por ativo</TabsTrigger></TabsList></Tabs>
    <Card className="shadow-none">
      ...
      <CardContent className="p-5 md:p-8">
        ...
        {step === 1 ? <StrategyStep rsiPeriod={rsiPeriod} rsiEntry={rsiEntry} rsiExit={rsiExit} maxBars={maxBars} hammer={hammer} management={management} onRsiPeriod={setRsiPeriod} onRsiEntry={setRsiEntry} onRsiExit={setRsiExit} onMaxBars={setMaxBars} onHammer={(changes) => setHammer((current) => ({ ...current, ...changes }))} onManagement={(changes) => setManagement((current) => ({ ...current, ...changes }))} /> : null}
...
type StrategyStepProps = {
  rsiPeriod: number; rsiEntry: number; rsiExit: number; maxBars: number; hammer: HammerSettings; management: PositionManagement;
  onRsiPeriod: (value: number) => void; onRsiEntry: (value: number) => void; onRsiExit: (value: number) => void; onMaxBars: (value: number) => void; onHammer: (changes: Partial<HammerSettings>) => void; onManagement: (changes: Partial<PositionManagement>) => void;
};
...
type ReviewProps = { mode: BacktestMode; collectionName: string; symbolCount: number; startDate: string; endDate: string; frequency: string; adjustment: string; rsiPeriod: number; rsiEntry: number; rsiExit: number; maxBars: number; hammer: HammerSettings; management: PositionManagement; initialCapital: number; maxPositions: number; pctPerTrade: number; restrictToIndex: boolean };
